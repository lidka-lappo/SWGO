...
def read_hv(folder_path, start_date=None, end_date=None):
    cols = [
        ...
        "RPCcrew_setHV_1", "RPCcrew_readHV_1", "RPCcrew_I_1",
        "RPCcrew_setHV_2", "RPCcrew_readHV_2", "RPCcrew_I_2"
    ]
    ...
    for fname in sorted(os.listdir(folder_path)):
        ...
        # Add RPC2 columns as copies of RPC1 columns
        df["RPC2_setHV_1"] = df["RPC1_setHV_1"]
        df["RPC2_readHV_1"] = df["RPC1_readHV_1"]
        df["RPC2_I_1"] = df["RPC1_I_1"]
        df["RPC2_setHV_2"] = df["RPC1_setHV_2"]
        df["RPC2_readHV_2"] = df["RPC1_readHV_2"]
        df["RPC2_I_2"] = df["RPC1_I_2"]

        all_dfs.append(df)

    if not all_dfs:
        print("⚠️ No HV data found in the given time period.")
        # Include RPC2 columns in empty dataframe
        all_cols = cols + [
            "RPC2_setHV_1", "RPC2_readHV_1", "RPC2_I_1",
            "RPC2_setHV_2", "RPC2_readHV_2", "RPC2_I_2"
        ]
        return pd.DataFrame(columns=all_cols).set_index("datetime")
